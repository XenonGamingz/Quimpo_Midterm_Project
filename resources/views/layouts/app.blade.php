...
<body class="bg-gray-100 font-sans">
  <div x-data="{ sidebarOpen: false }" class="min-h-screen flex">
    ...
    <div x-show="sidebarOpen" @click="sidebarOpen = false" class="fixed inset-0 z-30 bg-transparent md:hidden"></div>
    ...
    <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" class="fixed inset-y-0 left-0 z-40 w-64 bg-white shadow p-4 transform md:relative md:translate-x-0 transition-transform duration-200 ease-in-out">
      ...
      <nav>
        ...
        <a href="{{ route('library.books.trash') }}" class="block mt-1 py-2 px-3 rounded {{ request()->routeIs('library.books.trash') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700' }}">Books Trash</a>
        <a href="{{ route('library.categories.trash') }}" class="block mt-1 py-2 px-3 rounded {{ request()->routeIs('library.categories.trash') ? 'bg-indigo-50 text-indigo-700' : 'text-gray-700' }}">Categories Trash</a>
      </nav>
